...
def inception_block(bottom, name, weights, biases):
    ...
    with tf.name_scope(name + '1x1'):
        block['branch_1'] = conv_layer(bottom, weights['inception_' + name + '_1x1'],
                                       biases['inception_' + name + '_1x1'])

    with tf.name_scope(name + '3x3'):
        block['branch_2_r'] = conv_layer(bottom, weights['inception_' + name + '_3x3_reduce'],
                                         biases['inception_' + name + '_3x3_reduce'])
        block['branch_2'] = conv_layer(block['branch_2_r'], weights['inception_' + name + '_3x3'],
                                       biases['inception_' + name + '_3x3'])

    with tf.name_scope(name + '5x5'):
        block['branch_3_r'] = conv_layer(bottom, weights['inception_' + name + '_5x5_reduce'],
                                         biases['inception_' + name + '_5x5_reduce'])
        block['branch_3'] = conv_layer(block['branch_3_r'], weights['inception_' + name + '_5x5'],
                                       biases['inception_' + name + '_5x5'])

    with tf.name_scope(name + 'pool'):
        block['branch_4_p'] = max_pool(bottom)
        block['branch_4'] = conv_layer(block['branch_4_p'], weights['inception_' + name + '_pool_proj'],
                                       biases['inception_' + name + '_pool_proj'])

    block['concat'] = tf.concat(axis=3,
                                values=[block['branch_1'], block['branch_2'], block['branch_3'], block['branch_4']])
