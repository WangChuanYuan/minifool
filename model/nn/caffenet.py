...
def caffenet(input):
    ...
    vars = np.load('weight/caffenet.npy', allow_pickle=True, encoding='latin1').item()
    weights = {}
    biases = {}
    for name in vars.keys():
        weights[name] = tf.Variable(tf.constant(vars[name]['weights']), dtype='float32', name=name + "_weight",
                                    trainable=False)
        biases[name] = tf.Variable(tf.constant(vars[name]['biases']), dtype='float32', name=name + "_bias",
                                   trainable=False)
