...
def model(image, weights, biases):
    ...
    with tf.name_scope("conv2"):
        layers['conv2_3x3_reduce'] = conv_layer(layers['pool1_norm1'], weights['conv2_3x3_reduce'],
                                                biases['conv2_3x3_reduce'])
    ...
    with tf.name_scope('inception_5'):
        ...
        layers['pool5_7x7_s1'] = tf.nn.avg_pool(layers['inception_5b_output']['concat'], [1, 7, 7, 1], [1, 1, 1, 1],
                                                padding='VALID')
        layers['pool5_7x7_s1'] = tf.reshape(layers['pool5_7x7_s1'], [-1, 1024])

    with tf.name_scope('fc'):
        layers['loss3_classifier'] = fully_connected(layers['pool5_7x7_s1'], weights['loss3_classifier'],
                                                     biases['loss3_classifier'])
...
def googlenet(input):
    # weights and biases for tensorflow
    vars = np.load('weight/googlenet.npy').item()
    weights = {}
    biases = {}
    for name in vars.keys():
        weights[name] = tf.Variable(tf.constant(vars[name]['weights']), dtype='float32', name=name + '_weights',
                                    trainable=False)
        biases[name] = tf.Variable(tf.constant(vars[name]['biases']), dtype='float32', name=name + '_biases',
                                   trainable=False)
