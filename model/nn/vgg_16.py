...
def vgg16(input):
    ...
    vars = np.load('weight/vgg16.npy').item()
    weights = {}
    biases = {}
    for name in vars.keys():
        weights[name] = tf.Variable(tf.constant(vars[name][0]), dtype='float32', name=name + "_weight", trainable=False)
        biases[name] = tf.Variable(tf.constant(vars[name][1]), dtype='float32', name=name + "_bias", trainable=False)
